...
public class DynAttribsTest extends StrutsTestCase {
	...
	protected void setUp() throws Exception {
		...
		// look through classpath for the TLD file
		URLClassLoader loader 
			= (URLClassLoader)this.getClass().getClassLoader();
		URL[] urls = loader.getURLs();
		URL s2Url = null;
		for (URL url : urls) {
			String targetUrl = "struts2-core/target/classes/";
			if (! url.toString().contains(targetUrl)) {
				continue;
			}
			s2Url = url;
		}
		URI s2Uri = new URI(s2Url.toString() + "META-INF/struts-tags.tld");
		File tldFile = new File(s2Uri);
