...
_d91[1]="format";
return (_d8f||_d90[_d91.join("-")]).concat();
};
dojo.date.getDayName=function(_d92,_d93){
return dojo.date.getNames("days","wide","format",_d93)[_d92.getDay()];
};
dojo.date.getDayShortName=function(_d94,_d95){
return dojo.date.getNames("days","abbr","format",_d95)[_d94.getDay()];
};
dojo.date.getMonthName=function(_d96,_d97){
return dojo.date.getNames("months","wide","format",_d97)[_d96.getMonth()];
};
dojo.date.getMonthShortName=function(_d98,_d99){
return dojo.date.getNames("months","abbr","format",_d99)[_d98.getMonth()];
};
dojo.date.toRelativeString=function(_d9a){
var now=new Date();
var diff=(now-_d9a)/1000;
var end=" ago";
var _d9e=false;
if(diff<0){
_d9e=true;
...
if(diff==1){
return _d9e?"Tomorrow":"Yesterday";
}else{
return diff+" days"+end;
}
}
return dojo.date.format(_d9a);
};
dojo.date.toSql=function(_d9f,_da0){
return dojo.date.strftime(_d9f,"%F"+!_da0?" %T":"");
};
dojo.date.fromSql=function(_da1){
var _da2=_da1.split(/[\- :]/g);
while(_da2.length<6){
_da2.push(0);
}
...
},{isContainer:false,templatePath:dojo.uri.moduleUri("dojo.widget","templates/TimePicker.html"),templateCssPath:dojo.uri.moduleUri("dojo.widget","templates/TimePicker.css"),postMixInProperties:function(_da3,frag){
...
var _da7=this.getFragNodeRef(frag);
dojo.html.copyStyle(this.domNode,_da7);
if(args.value){
...
var _da8=dojo.widget.TimePicker.util.toAmPmHour(this.time.getHours());
var hour=_da8[0];
var isAm=_da8[1];
var _dab=this.time.getMinutes();
var _dac=parseInt(_dab/5);
this.onSetSelectedHour(this.hourIndexMap[hour]);
this.onSetSelectedMinute(this.minuteIndexMap[_dac]);
...
this.initData();
this.initUI();
},setDateTime:function(_dae){
this.storedTime=_dae;
...
for(var i=0;i<_db3.length;i++){
dojo.html.setClass(_db3.item(i),"");
}
},clearSelectedMinute:function(){
var _db5=this.minuteContainerNode.getElementsByTagName("td");
for(var i=0;i<_db5.length;i++){
dojo.html.setClass(_db5.item(i),"");
}
},clearSelectedAmPm:function(){
var _db7=this.amPmContainerNode.getElementsByTagName("td");
...
var _dbb=evt.target.parentNode;
}
dojo.event.browser.stopEvent(evt);
dojo.html.setClass(_dbb,this.classNames.selectedTime);
this.selectedTime["hour"]=_dbb.innerHTML;
}else{
if(!isNaN(evt)){
var _dbc=this.hourContainerNode.getElementsByTagName("td");
if(_dbc.item(evt)){
dojo.html.setClass(_dbc.item(evt),this.classNames.selectedTime);
this.selectedTime["hour"]=_dbc.item(evt).innerHTML;
}
}
}
this.selectedTime.anyTime=false;
...
var _dbf=evt.target.parentNode;
}
dojo.event.browser.stopEvent(evt);
dojo.html.setClass(_dbf,this.classNames.selectedTime);
this.selectedTime["minute"]=_dbf.innerHTML;
}else{
if(!isNaN(evt)){
var _dc0=this.minuteContainerNode.getElementsByTagName("td");
if(_dc0.item(evt)){
dojo.html.setClass(_dc0.item(evt),this.classNames.selectedTime);
this.selectedTime["minute"]=_dc0.item(evt).innerHTML;
}
}
}
...
var _dc3=evt.target;
if(evt&&_dc3){
if(_dc3.nodeType!=dojo.dom.ELEMENT_NODE){
_dc3=_dc3.parentNode;
}
dojo.event.browser.stopEvent(evt);
this.selectedTime.amPm=_dc3.id;
dojo.html.setClass(_dc3,this.classNames.selectedTime);
}else{
evt=evt?0:1;
var _dc4=this.amPmContainerNode.getElementsByTagName("td");
if(_dc4.item(evt)){
this.selectedTime.amPm=_dc4.item(evt).id;
dojo.html.setClass(_dc4.item(evt),this.classNames.selectedTime);
}
}
...
if(this.selectedTime.anyTime){
this.time=new Date();
var _dc8=dojo.widget.TimePicker.util.toRfcDateTime(this.time);
this.setDateTime(_dc8.split("T")[0]);
}else{
var hour=12;
var _dca=0;
var isAm=false;
...
_dca=parseInt(this.selectedTime["minute"],10);
}
...
this.toRfcDateTime=function(_dcd){
if(!_dcd){
_dcd=new Date();
}
_dcd.setSeconds(0);
return dojo.date.strftime(_dcd,"%Y-%m-%dT%H:%M:00%z");
};
this.fromRfcDateTime=function(_dce,_dcf,_dd0){
var _dd1=new Date();
if(!_dce||_dce.indexOf("T")==-1){
if(_dcf){
_dd1.setMinutes(Math.floor(_dd1.getMinutes()/5)*5);
}else{
_dd1.setMinutes(0);
}
}else{
var _dd2=_dce.split("T")[1].split(":");
var _dd1=new Date();
_dd1.setHours(_dd2[0]);
_dd1.setMinutes(_dd2[1]);
}
return _dd1;
};
...
this.fromAmPmHour=function(_dd6,isAm){
var hour=parseInt(_dd6,10);
if(isAm&&hour==12){
hour=0;
}else{
if(!isAm&&hour<12){
...
orig=c.join(":");
_dde=dojo.date.fromRfc3339(d+"T"+orig);
}
}
var _de3={widgetContainerId:this.widgetId,lang:this.lang,value:_dde};
this.timePicker=dojo.widget.createWidget("TimePicker",_de3,this.containerNode,"child");
...
},setValue:function(_de4){
this.setTime(_de4);
},setTime:function(_de5){
var _de6="";
if(_de5 instanceof Date){
_de6=_de5;
}else{
if(this.value){
var orig=this.value;
...
orig=c.join(":");
_de6=dojo.date.fromRfc3339(d+"T"+orig);
}
}
this.timePicker.setTime(_de6);
this._syncValueNode();
...
var _dec=dojo.string.trim(this.inputNode.value);
if(_dec){
var _ded=dojo.date.parse(_dec,{formatLength:this.formatLength,timePattern:this.displayFormat,selector:"timeOnly",locale:this.lang});
if(_ded){
this.setTime(_ded);
}
}else{
...
case "iso":
case "":
_def=dojo.date.toRfc3339(time,"timeOnly");
break;
case "posix":
case "unix":
_def=Number(time);
break;
default:
_def=dojo.date.format(time,{datePattern:this.saveFormat,selector:"timeOnly",locale:this.lang});
}
this.valueNode.value=_def;
},destroy:function(_df0){
this.timePicker.destroy(_df0);
...
break;
default:
_df2=dojo.date.format(time,{datePattern:this.saveFormat,selector:"timeOnly",locale:this.lang});
}
this.valueNode.value=_df2;
},_updateText:function(){
...
var _df4=this.valueNotifyTopicsArray[i];
if(!dojo.string.isBlank(_df4)){
try{
dojo.event.topic.publish(_df4,this.inputNode.value);
}
catch(ex){
dojo.debug(ex);
}
...
var _df7=this.getFragNodeRef(frag);
dojo.html.copyStyle(this.domNode,_df7);
this.weekTemplate=dojo.dom.removeNode(this.calendarWeekTemplate);
this._preInitUI(this.value?this.value:this.today,false,true);
var _df8=dojo.lang.unnest(dojo.date.getNames("days",this.dayWidth,"standAlone",this.lang));
if(this.weekStartsOn>0){
for(var i=0;i<this.weekStartsOn;i++){
_df8.push(_df8.shift());
}
}
var _dfa=this.dayLabelsRow.getElementsByTagName("td");
for(i=0;i<7;i++){
_dfa.item(i).innerHTML=_df8[i];
}
if(this.value){
...
this.setDate(_dfb);
},setDate:function(_dfc){
if(_dfc==""){
this.value="";
this._preInitUI(this.curMonth,false,true);
}else{
if(typeof _dfc=="string"){
this.value=dojo.date.fromRfc3339(_dfc);
this.value.setHours(0,0,0,0);
}else{
this.value=new Date(_dfc);
this.value.setHours(0,0,0,0);
}
}
...
},_preInitUI:function(_dfd,_dfe,_dff){
...
if(_dfd<this.startDate||_dfd>this.endDate){
_dfd=new Date((_dfd<this.startDate)?this.startDate:this.endDate);
}
this.firstDay=this._initFirstDay(_dfd,_dfe);
this.selectedIsUsed=false;
this.currentIsUsed=false;
var _e00=new Date(this.firstDay);
var _e01=_e00.getMonth();
this.curMonth=new Date(_e00);
this.curMonth.setDate(_e00.getDate()+6);
...
if(dojo.date.diff(_e00,this.endDate,dojo.date.dateParts.DAY)>days){
this._preInitUI(this.startDate,true,false);
_e00=new Date(this.firstDay);
}
this.curMonth=new Date(_e00);
this.curMonth.setDate(_e00.getDate()+6);
this.curMonth.setDate(1);
var _e03=(_e00.getMonth()==this.curMonth.getMonth())?"current":"previous";
}
if(_dff){
this._initUI(days);
}
...
var _e06=new Date(this.firstDay);
this._setMonthLabel(this.curMonth.getMonth());
this._setYearLabels(this.curMonth.getFullYear());
var _e07=this.calendarDatesContainerNode.getElementsByTagName("td");
var _e08=this.calendarDatesContainerNode.getElementsByTagName("tr");
var _e09;
for(i=0;i<days;i++){
_e09=_e07.item(i);
_e09.innerHTML=_e06.getDate();
_e09.setAttribute("djDateValue",_e06.valueOf());
var _e0a=(_e06.getMonth()!=this.curMonth.getMonth()&&Number(_e06)<Number(this.curMonth))?"previous":(_e06.getMonth()==this.curMonth.getMonth())?"current":"next";
var _e0b=_e0a;
if(this._isDisabledDate(_e06)){
var _e0c={previous:"disabledPrevious",current:"disabledCurrent",next:"disabledNext"};
_e0b=_e0c[_e0a];
}
dojo.html.setClass(_e09,this._getDateClassName(_e06,_e0b));
if(dojo.html.hasClass(_e09,this.classNames.selectedDate)){
this.selectedNode=_e09;
}
_e06=dojo.date.add(_e06,dojo.date.dateParts.DAY,1);
}
this.lastDay=dojo.date.add(_e06,dojo.date.dateParts.DAY,-1);
this._initControls();
},_initControls:function(){
var d=this.firstDay;
var d2=this.lastDay;
var _e0f,_e10,_e11,_e12,_e13,_e14;
_e0f=_e10=_e11=_e12=_e13=_e14=!this.staticDisplay;
with(dojo.date.dateParts){
var add=dojo.date.add;
if(_e0f&&add(d,DAY,(-1*(this._getAdjustedDay(d)+1)))<this.startDate){
_e0f=_e11=_e13=false;
}
if(_e10&&d2>this.endDate){
_e10=_e12=_e14=false;
}
if(_e11&&add(d,DAY,-1)<this.startDate){
_e11=_e13=false;
}
if(_e12&&add(d2,DAY,1)>this.endDate){
_e12=_e14=false;
}
if(_e13&&add(d2,YEAR,-1)<this.startDate){
_e13=false;
}
if(_e14&&add(d,YEAR,1)>this.endDate){
_e14=false;
}
}
function enableControl(node,_e17){
dojo.html.setVisibility(node,_e17?"":"hidden");
}
enableControl(this.decreaseWeekNode,_e0f);
enableControl(this.increaseWeekNode,_e10);
enableControl(this.decreaseMonthNode,_e11);
enableControl(this.increaseMonthNode,_e12);
enableControl(this.previousYearLabelNode,_e13);
enableControl(this.nextYearLabelNode,_e14);
},_incrementWeek:function(evt){
var d=new Date(this.firstDay);
switch(evt.target){
case this.increaseWeekNode.getElementsByTagName("img").item(0):
case this.increaseWeekNode:
var _e1a=dojo.date.add(d,dojo.date.dateParts.WEEK,1);
if(_e1a<this.endDate){
...
_e1d=dojo.date.add(_e1d,dojo.date.dateParts.DAY,this.displayWeeks*7);
if(_e1d<this.endDate){
...
var _e22=new Date(this.firstDay);
switch(evt.target){
case this.nextYearLabelNode:
_e22=dojo.date.add(_e22,dojo.date.dateParts.YEAR,1);
if(_e22<this.endDate){
year++;
}else{
var _e23=true;
}
...
},_setMonthLabel:function(_e29){
this.monthLabelNode.innerHTML=dojo.date.getNames("months","wide","standAlone",this.lang)[_e29];
...
f("current");
f("next");
},_getDateClassName:function(date,_e2f){
var _e30=this.classNames[_e2f];
if((!this.selectedIsUsed&&this.value)&&(Number(date)==Number(this.value))){
_e30=this.classNames.selectedDate+" "+_e30;
this.selectedIsUsed=true;
}
if((!this.currentIsUsed)&&(Number(date)==Number(this.today))){
_e30=_e30+" "+this.classNames.currentDate;
...
var _e33=evt.target;
if(_e33.nodeType!=dojo.dom.ELEMENT_NODE){
_e33=_e33.parentNode;
}
dojo.event.browser.stopEvent(evt);
this.selectedIsUsed=this.todayIsUsed=false;
if(dojo.html.hasClass(_e33,this.classNames["disabledPrevious"])||dojo.html.hasClass(_e33,this.classNames["disabledCurrent"])||dojo.html.hasClass(_e33,this.classNames["disabledNext"])){
return;
}
this.clickedNode=_e33;
this.setDate(new Date(Number(dojo.html.getAttribute(_e33,"djDateValue"))));
},onValueChanged:function(date){
},_isDisabledDate:function(_e35){
if(_e35<this.startDate||_e35>this.endDate){
return true;
}
return this.isDisabledDate(_e35,this.lang);
},isDisabledDate:function(_e36,_e37){
return false;
},_initFirstDay:function(_e38,adj){
var d=new Date(_e38);
if(!adj){
d.setDate(1);
}
...
var _e44={widgetContainerId:this.widgetId,lang:this.lang,value:this.value,startDate:this.startDate,endDate:this.endDate,displayWeeks:this.displayWeeks,weekStartsOn:this.weekStartsOn,adjustWeeks:this.adjustWeeks,staticDisplay:this.staticDisplay};
this.datePicker=dojo.widget.createWidget("DatePicker",_e44,this.containerNode,"child");
...
},setValue:function(_e45){
this.setDate(_e45);
},setDate:function(_e46){
this.datePicker.setDate(_e46);
this._syncValueNode();
},_updateText:function(){
...
this.hideContainer();
},onValueChanged:function(_e47){
},onInputChange:function(){
var _e48=dojo.string.trim(this.inputNode.value);
if(_e48){
var _e49=dojo.date.parse(_e48,{formatLength:this.formatLength,datePattern:this.displayFormat,selector:"dateOnly",locale:this.lang});
if(!this.datePicker._isDisabledDate(_e49)){
this.setDate(_e49);
}
}else{
if(_e48==""){
...
case "iso":
case "":
_e4b=dojo.date.toRfc3339(date,"dateOnly");
break;
case "posix":
case "unix":
_e4b=Number(date);
...
default:
if(date){
_e4b=dojo.date.format(date,{datePattern:this.saveFormat,selector:"dateOnly",locale:this.lang});
}
}
this.valueNode.value=_e4b;
},destroy:function(_e4c){
this.datePicker.destroy(_e4c);
...
if(this.valueNotifyTopicsArray!=null){
for(var i=0;i<this.valueNotifyTopicsArray.length;i++){
var _e4e=this.valueNotifyTopicsArray[i];
if(!dojo.string.isBlank(_e4e)){
try{
dojo.event.topic.publish(_e4e,this.inputNode.value);
}
catch(ex){
dojo.debug(ex);
}
...
var _e50=this.events.split(",");
var _e51=this.sources.split(",");
if(_e50&&this.domNode){
dojo.lang.forEach(_e50,function(_e52){
dojo.lang.forEach(_e51,function(_e53){
var _e54=dojo.byId(_e53);
if(_e54){
dojo.event.connect(_e54,_e52,function(evt){
evt.preventDefault();
evt.stopPropagation();
self.reloadContents();
});
...
},onTreeDestroy:function(_e59){
this.unlistenTree(_e59.source);
if(this.dieWithTree){
this.destroy();
}
},onCollapse:function(_e5a){
if(!this.selectedNode){
return;
}
var node=_e5a.source;
var _e5c=this.selectedNode.parent;
while(_e5c!==node&&_e5c.isTreeNode){
_e5c=_e5c.parent;
}
if(_e5c.isTreeNode){
this.deselect();
}
},select:function(_e5d){
var node=_e5d.source;
var e=_e5d.event;
...
},onMoveFrom:function(_e60){
if(_e60.child!==this.selectedNode){
return;
}
if(!dojo.lang.inArray(this.listenedTrees,_e60.newTree)){
this.deselect();
}
},onRemoveNode:function(_e61){
...
},publishTopics:function(_e66,node){
if(_e66!=null){
for(var i=0;i<_e66.length;i++){
var _e69=_e66[i];
if(!dojo.string.isBlank(_e69)){
try{
dojo.event.topic.publish(_e69,node);
}
catch(ex){
dojo.debug(ex);
}
...
var _e73=this.getFragNodeRef(frag);
dojo.html.copyStyle(this.domNode,_e73);
...
},selectChild:function(page,_e7c){
page=dojo.widget.byId(page);
this.correspondingPageButton=_e7c;
if(this.selectedChildWidget){
...
var _e7d=dojo.lang.find(this.children,this.selectedChildWidget);
this.selectChild(this.children[_e7d+1]);
},back:function(){
var _e7e=dojo.lang.find(this.children,this.selectedChildWidget);
this.selectChild(this.children[_e7e-1]);
},onResized:function(){
...
var _e7f=dojo.html.getContentBox(this.containerNode);
this.selectedChildWidget.resizeTo(_e7f.width,_e7f.height);
}
},_showChild:function(page){
if(this.doLayout){
var _e81=dojo.html.getContentBox(this.containerNode);
page.resizeTo(_e81.width,_e81.height);
}
page.selected=true;
page.show();
...
var _e84=page.onClose(this,page);
if(_e84){
...
this.pane2button={};
var _e85=dojo.widget.byId(this.containerId);
if(_e85){
dojo.lang.forEach(_e85.children,this.onAddChild,this);
}
...
var _e87=dojo.widget.createWidget(this.buttonWidget,{label:page.label,closeButton:page.closable});
this.addChild(_e87);
this.domNode.appendChild(_e87.domNode);
this.pane2button[page]=_e87;
page.controlButton=_e87;
var _e88=this;
dojo.event.connect(_e87,"onClick",function(){
_e88.onButtonClick(page);
});
dojo.event.connect(_e87,"onCloseButtonClick",function(){
_e88.onCloseButtonClick(page);
});
},onRemoveChild:function(page){
if(this._currentChild==page){
this._currentChild=null;
}
var _e8a=this.pane2button[page];
if(_e8a){
_e8a.destroy();
}
...
var _e8c=this.pane2button[this._currentChild];
_e8c.clearSelected();
}
var _e8d=this.pane2button[page];
_e8d.setSelected();
this._currentChild=page;
},onButtonClick:function(page){
var _e8f=dojo.widget.byId(this.containerId);
_e8f.selectChild(page,false,this);
},onCloseButtonClick:function(page){
var _e91=dojo.widget.byId(this.containerId);
_e91.closeChild(page);
},onKey:function(evt){
if((evt.keyCode==evt.KEY_RIGHT_ARROW)||(evt.keyCode==evt.KEY_LEFT_ARROW)){
var _e93=0;
var next=null;
var _e93=dojo.lang.find(this.children,this.pane2button[this._currentChild]);
if(evt.keyCode==evt.KEY_RIGHT_ARROW){
next=this.children[(_e93+1)%this.children.length];
}else{
next=this.children[(_e93+(this.children.length-1))%this.children.length];
}
dojo.event.browser.stopEvent(evt);
next.onClick();
}
...
var _e99=this.labelPosition.replace(/-h/,"");
var _e9a=[{domNode:this.tablist.domNode,layoutAlign:_e99},{domNode:this.containerNode,layoutAlign:"client"}];
dojo.widget.html.layout(this.domNode,_e9a);
if(this.selectedChildWidget){
var _e9b=dojo.html.getContentBox(this.containerNode);
this.selectedChildWidget.resizeTo(_e9b.width,_e9b.height);
}
},selectTab:function(tab,_e9d){
dojo.deprecated("use selectChild() rather than selectTab(), selectTab() will be removed in","0.5");
this.selectChild(tab,_e9d);
},onKey:function(e){
if(e.keyCode==e.KEY_UP_ARROW&&e.ctrlKey){
var _e9f=this.correspondingTabButton||this.selectedTabWidget.tabButton;
_e9f.focus();
dojo.event.browser.stopEvent(e);
}else{
...
},selectChild:function(tab,_ea3){
if(!tab.disabled){
var _ea4={"cancel":false};
if(this.beforeSelectTabNotifyTopicsArray){
var self=this;
dojo.lang.forEach(this.beforeSelectTabNotifyTopicsArray,function(_ea6){
try{
dojo.event.topic.publish(_ea6,self,tab,_ea4);
}
catch(ex){
dojo.debug(ex);
}
...
if(!_ea4.cancel){
struts.widget.StrutsTabContainer.superclass.selectChild.apply(this,[tab,_ea3]);
if(this.afterSelectTabNotifyTopicsArray){
var self=this;
dojo.lang.forEach(this.afterSelectTabNotifyTopicsArray,function(_ea7){
try{
dojo.event.topic.publish(_ea7,self,tab,_ea4);
}
catch(ex){
dojo.debug(ex);
}
...
var _ea9=this.getTabWidget(t);
_ea9.disabled=true;
dojo.html.addClass(_ea9.controlButton.domNode,this.disabledTabCssClass);
},enableTab:function(t){
var _eab=this.getTabWidget(t);
_eab.disabled=false;
dojo.html.removeClass(_eab.controlButton.domNode,this.disabledTabCssClass);
...
},actionIsDisabled:function(_ead){
var _eae=this;
var _eaf=false;
if(this.tree.strictFolders&&_ead==this.actions.ADDCHILD&&!this.isFolder){
_eaf=true;
}
if(dojo.lang.inArray(_eae.actionsDisabled,_ead)){
_eaf=true;
}
if(this.isLocked()){
_eaf=true;
...
},adjustDepth:function(_eb4){
for(var i=0;i<this.children.length;i++){
this.children[i].adjustDepth(_eb4);
}
this.depth+=_eb4;
if(_eb4>0){
for(var i=0;i<_eb4;i++){
...
this.isFolder=true;
},createDOMNode:function(tree,_eb9){
this.tree=tree;
this.depth=_eb9;
this.imgs=[];
...
var _ec0=this.parent;
for(var i=0;i<this.depth;i++){
var idx=this.imgs.length-(3+i);
var img=(this.tree.showGrid&&!_ec0.isLastChild())?this.tree.gridIconSrcV:this.tree.blankIconSrc;
...
dojo.lang.forEach(_ec4.getDescendants(),function(node){
node.setGridImage(_ec4.depth,icon);
});
...
var self=this;
var _ecb=dojo.lang.toArray(this.children);
dojo.lang.forEach(_ecb,function(node){
...
this.removeChildren();
var _ece=href+(href.indexOf("?")>-1?"&":"?")+"nodeId="+this.widgetId;
var self=this;
this.markLoading();
dojo.io.bind({url:_ece,useCache:false,preventCache:true,handler:function(type,data,e){
if(type=="load"){
if(data){
dojo.lang.forEach(data,function(_ed3){
var _ed4=dojo.widget.createWidget("struts:StrutsTreeNode",{title:_ed3.label,isFolder:_ed3.hasChildren,widgetId:_ed3.id});
self.addChild(_ed4);
});
}
}
...
dojo.json={jsonRegistry:new dojo.AdapterRegistry(),register:function(name,_ed6,wrap,_ed8){
dojo.json.jsonRegistry.register(name,_ed6,wrap,_ed8);
...
if(_edb!="function"&&typeof (o.length)=="number"){
var res=[];
...
try{
window.o=o;
_edd=dojo.json.jsonRegistry.match(o);
return me(_edd);
}
catch(e){
}
...
if(typeof (k)=="string"){
_ee2=dojo.string.escapeString(k);
}else{
continue;
}
}
...
dojo.dnd.TreeDragSource=function(node,_ee4,type,_ee6){
this.controller=_ee4;
this.treeNode=_ee6;
dojo.dnd.HtmlDragSource.call(this,node,type);
};
dojo.inherits(dojo.dnd.TreeDragSource,dojo.dnd.HtmlDragSource);
dojo.lang.extend(dojo.dnd.TreeDragSource,{onDragStart:function(){
var _ee7=dojo.dnd.HtmlDragSource.prototype.onDragStart.call(this);
_ee7.treeNode=this.treeNode;
_ee7.onDragStart=dojo.lang.hitch(_ee7,function(e){
this.savedSelectedNode=this.treeNode.tree.selector.selectedNode;
if(this.savedSelectedNode){
this.savedSelectedNode.unMarkSelected();
}
var _ee9=dojo.dnd.HtmlDragObject.prototype.onDragStart.apply(this,arguments);
var _eea=this.dragClone.getElementsByTagName("img");
for(var i=0;i<_eea.length;i++){
_eea.item(i).style.backgroundImage="url()";
}
return _ee9;
});
_ee7.onDragEnd=function(e){
if(this.savedSelectedNode){
...
dojo.dnd.TreeDropTarget=function(_eef,_ef0,type,_ef2){
this.treeNode=_ef2;
this.controller=_ef0;
dojo.dnd.HtmlDropTarget.apply(this,[_eef,type]);
};
dojo.inherits(dojo.dnd.TreeDropTarget,dojo.dnd.HtmlDropTarget);
dojo.lang.extend(dojo.dnd.TreeDropTarget,{autoExpandDelay:1500,autoExpandTimer:null,position:null,indicatorStyle:"2px black solid",showIndicator:function(_ef3){
if(this.position==_ef3){
return;
}
this.hideIndicator();
this.position=_ef3;
...
var _ef5=dojo.dnd.HtmlDropTarget.prototype.onDragOver.apply(this,arguments);
if(_ef5&&this.treeNode.isFolder&&!this.treeNode.isExpanded){
this.setAutoExpandTimer();
}
return _ef5;
},accepts:function(_ef6){
var _ef7=dojo.dnd.HtmlDropTarget.prototype.accepts.apply(this,arguments);
if(!_ef7){
return false;
}
var _ef8=_ef6[0].treeNode;
if(dojo.lang.isUndefined(_ef8)||!_ef8||!_ef8.isTreeNode){
dojo.raise("Source is not TreeNode or not found");
}
if(_ef8===this.treeNode){
return false;
}
return true;
...
var _ef9=this;
var _efa=function(){
if(dojo.dnd.dragManager.currentDropTarget===_ef9){
_ef9.controller.expand(_ef9.treeNode);
}
};
this.autoExpandTimer=dojo.lang.setTimeout(_efa,_ef9.autoExpandDelay);
},getDNDMode:function(){
return this.treeNode.tree.DNDMode;
},getAcceptPosition:function(e,_efc){
var _efd=this.getDNDMode();
if(_efd&dojo.widget.Tree.prototype.DNDModes.ONTO&&!(!this.treeNode.actionIsDisabled(dojo.widget.TreeNode.prototype.actions.ADDCHILD)&&_efc.parent!==this.treeNode&&this.controller.canMove(_efc,this.treeNode))){
_efd&=~dojo.widget.Tree.prototype.DNDModes.ONTO;
}
var _efe=this.getPosition(e,_efd);
if(_efe=="onto"||(!this.isAdjacentNode(_efc,_efe)&&this.controller.canMove(_efc,this.treeNode.parent))){
return _efe;
}else{
return false;
}
...
},onDragMove:function(e,_f01){
var _f02=_f01[0].treeNode;
var _f03=this.getAcceptPosition(e,_f02);
if(_f03){
this.showIndicator(_f03);
}
},isAdjacentNode:function(_f04,_f05){
if(_f04===this.treeNode){
return true;
}
if(_f04.getNextSibling()===this.treeNode&&_f05=="before"){
return true;
}
if(_f04.getPreviousSibling()===this.treeNode&&_f05=="after"){
return true;
}
return false;
},getPosition:function(e,_f07){
var node=dojo.byId(this.treeNode.labelNode);
var _f09=e.pageY||e.clientY+dojo.body().scrollTop;
var _f0a=dojo.html.getAbsolutePosition(node).y;
var _f0b=dojo.html.getBorderBox(node).height;
var relY=_f09-_f0a;
var p=relY/_f0b;
var _f0e="";
if(_f07&dojo.widget.Tree.prototype.DNDModes.ONTO&&_f07&dojo.widget.Tree.prototype.DNDModes.BETWEEN){
if(p<=0.3){
_f0e="before";
}else{
if(p<=0.7){
...
if(_f07&dojo.widget.Tree.prototype.DNDModes.BETWEEN){
if(p<=0.5){
_f0e="before";
}else{
_f0e="after";
...
if(_f07&dojo.widget.Tree.prototype.DNDModes.ONTO){
_f0e="onto";
}
}
}
return _f0e;
},getTargetParentIndex:function(_f0f,_f10){
var _f11=_f10=="before"?this.treeNode.getParentIndex():this.treeNode.getParentIndex()+1;
if(this.treeNode.parent===_f0f.parent&&this.treeNode.getParentIndex()>_f0f.getParentIndex()){
_f11--;
}
return _f11;
},onDrop:function(e){
var _f13=this.position;
this.onDragOut(e);
var _f14=e.dragObject.treeNode;
if(!dojo.lang.isObject(_f14)){
dojo.raise("TreeNode not found in dragObject");
}
if(_f13=="onto"){
return this.controller.move(_f14,this.treeNode,0);
}else{
var _f15=this.getTargetParentIndex(_f14,_f13);
return this.controller.move(_f14,this.treeNode.parent,_f15);
}
}});
dojo.dnd.TreeDNDController=function(_f16){
this.treeController=_f16;
this.dragSources={};
this.dropTargets={};
};
...
dojo.event.topic.unsubscribe(tree.eventNames.removeNode,this,"onRemoveNode");
dojo.event.topic.unsubscribe(tree.eventNames.treeDestroy,this,"onTreeDestroy");
},onTreeDestroy:function(_f19){
this.unlistenTree(_f19.source);
},onCreateDOMNode:function(_f1a){
this.registerDNDNode(_f1a.source);
},onAddChild:function(_f1b){
this.registerDNDNode(_f1b.child);
},onMoveFrom:function(_f1c){
var _f1d=this;
dojo.lang.forEach(_f1c.child.getDescendants(),function(node){
_f1d.unregisterDNDNode(node);
});
},onMoveTo:function(_f1f){
var _f20=this;
dojo.lang.forEach(_f1f.child.getDescendants(),function(node){
_f20.registerDNDNode(node);
});
...
var _f23=new dojo.dnd.TreeDragSource(node.labelNode,this,node.tree.widgetId,node);
this.dragSources[node.widgetId]=_f23;
}
var _f24=new dojo.dnd.TreeDropTarget(node.labelNode,this.treeController,node.tree.DNDAcceptTypes,node);
this.dropTargets[node.widgetId]=_f24;
...
var tree=_f2a.source;
this.unlistenTree(tree);
if(this.dieWithTree){
this.destroy();
}
},onCreateDOMNode:function(_f2c){
var node=_f2c.source;
if(node.expandLevel>0){
this.expandToLevel(node,node.expandLevel);
}
},onTreeCreate:function(_f2e){
var tree=_f2e.source;
var _f30=this;
if(tree.expandLevel){
dojo.lang.forEach(tree.children,function(_f31){
_f30.expandToLevel(_f31,tree.expandLevel-1);
});
}
},expandToLevel:function(node,_f33){
if(_f33==0){
return;
}
var _f34=node.children;
var _f35=this;
var _f36=function(node,_f38){
this.node=node;
this.expandLevel=_f38;
this.process=function(){
for(var i=0;i<this.node.children.length;i++){
var _f3a=node.children[i];
_f35.expandToLevel(_f3a,this.expandLevel);
}
};
};
var h=new _f36(node,_f33-1);
this.expand(node,false,h,h.process);
},onTreeClick:function(_f3c){
...
},expand:function(node,sync,_f40,_f41){
node.expand();
if(_f41){
_f41.apply(_f40,[node]);
}
},collapse:function(node){
node.collapse();
},canMove:function(_f43,_f44){
if(_f43.actionIsDisabled(_f43.actions.MOVE)){
return false;
}
if(_f43.parent!==_f44&&_f44.actionIsDisabled(_f44.actions.ADDCHILD)){
return false;
}
var node=_f44;
...
return true;
},move:function(_f46,_f47,_f48){
if(!this.canMove(_f46,_f47)){
return false;
}
var _f49=this.doMove(_f46,_f47,_f48);
if(!_f49){
return _f49;
}
if(_f47.isTreeNode){
...
return _f49;
},doMove:function(_f4a,_f4b,_f4c){
_f4a.tree.move(_f4a,_f4b,_f4c);
return true;
},canRemoveNode:function(_f4d){
if(_f4d.actionIsDisabled(_f4d.actions.REMOVE)){
return false;
}
return true;
},removeNode:function(node,_f4f,_f50){
if(!this.canRemoveNode(node)){
return false;
}
return this.doRemoveNode(node,_f4f,_f50);
},doRemoveNode:function(node,_f52,_f53){
node.tree.removeNode(node);
if(_f53){
_f53.apply(dojo.lang.isUndefined(_f52)?this:_f52,[node]);
}
},canCreateChild:function(_f54,_f55,data){
if(_f54.actionIsDisabled(_f54.actions.ADDCHILD)){
return false;
}
return true;
},createChild:function(_f57,_f58,data,_f5a,_f5b){
if(!this.canCreateChild(_f57,_f58,data)){
return false;
}
return this.doCreateChild.apply(this,arguments);
},doCreateChild:function(_f5c,_f5d,data,_f5f,_f60){
var _f61=data.widgetType?data.widgetType:"TreeNode";
var _f62=dojo.widget.createWidget(_f61,data);
_f5c.addChild(_f62,_f5d);
this.expand(_f5c);
if(_f60){
_f60.apply(_f5f,[_f62]);
}
return _f62;
}});
...
return dojo.widget.HtmlWidget.prototype.destroy.apply(this,arguments);
},addChild:function(_f6c,_f6d){
var _f6e={child:_f6c,index:_f6d,parent:this,domNodeInitialized:_f6c.domNodeInitialized};
this.doAddChild.apply(this,arguments);
dojo.event.topic.publish(this.tree.eventNames.addChild,_f6e);
},doAddChild:function(_f6f,_f70){
if(dojo.lang.isUndefined(_f70)){
_f70=this.children.length;
}
if(!_f6f.isTreeNode){
...
var _f71=this;
dojo.lang.forEach(_f6f.getDescendants(),function(elem){
elem.tree=_f71.tree;
});
_f6f.parent=this;
if(this.isTreeNode){
...
if(_f70<this.children.length){
dojo.html.insertBefore(_f6f.domNode,this.children[_f70].domNode);
}else{
this.containerNode.appendChild(_f6f.domNode);
if(this.isExpanded&&this.isTreeNode){
this.showChildren();
}
}
this.children.splice(_f70,0,_f6f);
if(_f6f.domNodeInitialized){
var d=this.isTreeNode?this.depth:-1;
_f6f.adjustDepth(d-_f6f.depth+1);
_f6f.updateIconTree();
}else{
_f6f.depth=this.isTreeNode?this.depth+1:0;
_f6f.createDOMNode(_f6f.tree,_f6f.depth);
}
var _f74=_f6f.getPreviousSibling();
if(_f6f.isLastChild()&&_f74){
_f74.updateExpandGridColumn();
}
...
},move:function(_f77,_f78,_f79){
var _f7a=_f77.parent;
var _f7b=_f77.tree;
this.doMove.apply(this,arguments);
var _f78=_f77.parent;
var _f7c=_f77.tree;
var _f7d={oldParent:_f7a,oldTree:_f7b,newParent:_f78,newTree:_f7c,child:_f77};
dojo.event.topic.publish(_f7b.eventNames.moveFrom,_f7d);
dojo.event.topic.publish(_f7c.eventNames.moveTo,_f7d);
},doMove:function(_f7e,_f7f,_f80){
_f7e.parent.doRemoveNode(_f7e);
_f7f.doAddChild(_f7e,_f80);
},removeNode:function(_f81){
if(!_f81.parent){
return;
}
var _f82=_f81.tree;
var _f83=_f81.parent;
var _f84=this.doRemoveNode.apply(this,arguments);
dojo.event.topic.publish(this.tree.eventNames.removeNode,{child:_f84,tree:_f82,parent:_f83});
return _f84;
},doRemoveNode:function(_f85){
if(!_f85.parent){
return;
}
var _f86=_f85.parent;
var _f87=_f86.children;
var _f88=_f85.getParentIndex();
if(_f88<0){
dojo.raise("Couldn't find node "+_f85+" for removal");
}
_f87.splice(_f88,1);
dojo.html.removeNode(_f85.domNode);
if(_f86.children.length==0&&!_f86.isTree){
_f86.containerNode.style.display="none";
}
if(_f88==_f87.length&&_f88>0){
_f87[_f88-1].updateExpandGridColumn();
}
if(_f86 instanceof dojo.widget.Tree&&_f88==0&&_f87.length>0){
_f87[0].updateExpandGrid();
}
_f85.parent=_f85.tree=null;
return _f85;
},markLoading:function(){
},unMarkLoading:function(){
},lock:function(){
...
if(type=="load"){
if(data){
dojo.lang.forEach(data,function(_f8e){
var _f8f=dojo.widget.createWidget("struts:StrutsTreeNode",{title:_f8e.label,isFolder:_f8e.hasChildren,widgetId:_f8e.id});
self.addChild(_f8f);
});
}
}else{
if(self.errorNotifyTopicsArray){
dojo.lang.forEach(self.errorNotifyTopicsArray,function(_f90){
try{
dojo.event.topic.publish(_f90,data,e,self);
}
catch(ex){
dojo.debug(ex);
}
